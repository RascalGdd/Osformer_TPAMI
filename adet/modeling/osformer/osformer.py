...
from .trans_decoder_V2 import build_transformer_decoder_v2
from .trans_encoder import build_transformer_encoder
from .position_encoding import build_position_encoding
from .instance_fusion import DCIN
from .loss_sparse import build_sparse_inst_criterion
...
class OSFormer(nn.Module):
    def __init__(self, cfg):
        ...
        # inference
        self.cls_threshold = cfg.MODEL.OSFormer.CLS_THRESHOLD
        self.mask_threshold = cfg.MODEL.OSFormer.MASK_THRESHOLD
        self.max_detections = cfg.MODEL.OSFormer.MAX_DETECTIONS
    ...
    def forward(self, batched_inputs):
     ...
     #   pred_masks = self.dcin(kernel_pred, mask_pred)
        ...
        if self.training:
            ...
            losses = self.criterion(output, targets, max_shape, sem_targets, sem_pred)
            ...
            results = self.inference(
                output, batched_inputs, max_shape, images.image_sizes, sem_pred)
    ...
    def inference(self, output, batched_inputs, max_shape, image_sizes, pred_sems=None):
...
class CISTransformerHead(nn.Module):
    def __init__(self, cfg, input_shape: List[ShapeSpec]):
        ...
        self.trans_decoder = build_transformer_decoder_v2(cfg)
        ...
        torch.nn.init.constant_(self.kernel_pred.bias, bias_value)

        #Efficient location-guided query
        self.num_masks = cfg.MODEL.OSFormer.QS.NUM_MASKS  # 100
        # self.query_embed = nn.Embedding(self.num_masks, self.hidden_dim)
        self.reduce_channel = nn.Conv2d(self.instance_in_channels, self.num_masks, 3, padding=1)
    ...
    def forward(self, features, old_features, mask_head):
        ...
        for idx, feature in enumerate(features):
            ...
            pos_encoders.append(position_embedding)
            bs, _, w, h = feature.shape
            sizes_encoder.append((w, h))

        split_list = [w * h for (w, h) in sizes_encoder]
        trans_memory = []
        memorys, level_start_index = self.trans_encoder(features, pos_encoders)
        ...
        for memory, (w, h), seg_num_grid, pos_encoder in zip(memorys.split(split_list, 1),
            ...
            # feat = F.interpolate(ins_kernel_feat, size=10, mode='bilinear')  # (bs, c, w, h)
            ...
            # srcs_feature = query_feature.unsqueeze(0).expand(bs, -1, -1)  # (bs, num_queries, hidden_dim)
            ...
            # pos_query = torch.zeros((bs, num_channel, C), device=query_feature.device)
            # pos_queries.append(pos_query)
